...
void initialize_charging (void)
{
	...
	if((confparam[CONF_PS_SCHEME].value == BAT_BOOST_BUS_SCHEME)||(confparam[CONF_PS_SCHEME].value == AC_OR_BATBOOST_SCHEME))
	{
		...
		if((x%2) != 0)	x++;	//we want x to be even
...
{	//this gets called from analogs.c when the ADC data set is ready, 8khz rep rate
    uint32 v_threshold;
    ...
    if(bus_command == BUS_COMMAND_ON)
    {
		...
		else if(confparam[CONF_PS_SCHEME].value == AC_PRECHARGE_BUS_SCHEME)
		{
			...
			if(charging_counter == 0)	initial_vbus = telemetry.bus_v;
			charging_counter++;
		...
		else if(confparam[CONF_PS_SCHEME].value == AC_OR_BATBOOST_SCHEME)
		{
			...
			if(charging_counter > CHARGE_TEST_TIMEOUT)	
			{
				final_vbus = telemetry.bus_v;
				delta_vbus = final_vbus - initial_vbus;
				if(delta_vbus > 4)	confparam[CONF_PS_SCHEME].value = AC_PRECHARGE_BUS_SCHEME;
				...
				else bus_command = BUS_COMMAND_FAULT;
				charging_counter = 0;
			}
		}
    }
