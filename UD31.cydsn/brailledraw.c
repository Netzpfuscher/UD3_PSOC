/*
 * Copyright (c) 2014 Steve Ward
 * Copyright (c) 2018 Jens Kerrinnes
 * LICENCE: MIT License (look at /LICENCE.md)
 */

/* [] END OF FILE */
#include "brailledraw.h"
#include <stdlib.h>
#include "cli_common.h"


const unsigned char pixmap[4][2] = {
    {0x01, 0x08},
    {0x02, 0x10},
    {0x04, 0x20},
    {0x40, 0x80}
};


uint8_t pix_buffer[PIX_WIDTH/8][PIX_HEIGHT];
char out_buffer[4]={0x00,0x00,0x00,0x00};

static void set_bytes(char* pbuffer, const unsigned char c) {
    pbuffer[0] = (char)0xE2;
    if (c & pixmap[3][0] && c & pixmap[3][1]) {
        pbuffer[1] = (char)0xA3;
    } else if (c & pixmap[3][1]) {
        pbuffer[1] = (char)0xA2;
    } else if (c & pixmap[3][0]) {
        pbuffer[1] = (char)0xA1;
    } else {
        pbuffer[1] = (char)0xA0;
    }

    pbuffer[2] = (char)((0xBF & c) | 0x80);

}

void braille_setPixel(uint8_t x, uint8_t y){
    if(x>PIX_WIDTH-1) x=PIX_WIDTH-1;
    if(y>PIX_HEIGHT-1) y=PIX_HEIGHT-1;
	uint8_t byte = x/8;
	uint8_t bit = x%8;
	
	pix_buffer[byte][y] |= 1 << bit;

}

void braille_line(int x0, int y0, int x1, int y1){
 
  int dx = abs(x1-x0), sx = x0<x1 ? 1 : -1;
  int dy = abs(y1-y0), sy = y0<y1 ? 1 : -1; 
  int err = (dx>dy ? dx : -dy)/2, e2;
 
  for(;;){
    braille_setPixel(x0,y0);
    if (x0==x1 && y0==y1) break;
    e2 = err;
    if (e2 >-dx) { err -= dy; x0 += sx; }
    if (e2 < dy) { err += dx; y0 += sy; }
  }
}

void braille_clear(void){
    for(uint8_t x=0;x<PIX_WIDTH/8;x++){
        for(uint8_t y=0;y<PIX_HEIGHT;y++){
               pix_buffer[x][y]=0x00;
        }
    }
}

void braille_draw(uint8_t port) {
	unsigned char byte;
	for(uint16_t y=0;y<PIX_HEIGHT;y+=4){
	    for(uint16_t x=0;x<PIX_WIDTH/8;x++){
	        for(uint16_t bpos = 0;bpos<8;bpos+=2){
        		byte=0x00;
        		if (pix_buffer[x][y] & (1 << bpos)) byte |= 1 << 0;
        		if (pix_buffer[x][y] & (1 << (bpos+1))) byte |= 1 << 3;
        		
        		if (pix_buffer[x][y+1] & (1 << bpos)) byte |= 1 << 1;
        		if (pix_buffer[x][y+1] & (1 << (bpos+1))) byte |= 1 << 4;
        		
        		if (pix_buffer[x][y+2] & (1 << bpos)) byte |= 1 << 2;
        		if (pix_buffer[x][y+2] & (1 << (bpos+1))) byte |= 1 << 5;
        		
        		if (pix_buffer[x][y+3] & (1 << bpos)) byte |= 1 << 6;
        		if (pix_buffer[x][y+3] & (1 << (bpos+1))) byte |= 1 << 7;
        		
        		set_bytes(out_buffer, byte);
        		send_string(out_buffer, port);
        	}
    	}
    send_string("\r\n", port);
	}	
}
